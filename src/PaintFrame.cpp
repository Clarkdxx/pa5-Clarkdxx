...
{
	// TODO
    wxFileDialog saveFileDialog(this, _("Save file"), "", "", "PNG files (*.png)|*.png|BMP files (*.bmp)|*.bmp|JPEG files (*.jpeg)|*.jpeg", wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
    ...
    {
        mModel->SaveBitMap(fileName, mPanel->GetSize(), wxBITMAP_TYPE_PNG);
    }
    else if(fileName.substr(fileName.find_last_of(".") + 1) == "bmp")
    {
        mModel->SaveBitMap(fileName, mPanel->GetSize(), wxBITMAP_TYPE_BMP);
    }
    else if(fileName.substr(fileName.find_last_of(".") + 1) == "jpeg")
    {
        mModel->SaveBitMap(fileName, mPanel->GetSize(), wxBITMAP_TYPE_JPEG);
    }
}
...
{
	// TODO
    wxFileDialog openFileDialog(this, _("Open file"), "", "", "PNG files (*.png)|*.png|BMP files (*.bmp)|*.bmp|JPEG files (*.jpeg)|*.jpeg", wxFD_OPEN|wxFD_FILE_MUST_EXIST);
    ...
    if(fileName.substr(fileName.find_last_of(".") + 1) == "png")
    {
        mModel->LoadBitMap(fileName, wxBITMAP_TYPE_PNG);
    }
    else if(fileName.substr(fileName.find_last_of(".") + 1) == "bmp")
    {
        mModel->LoadBitMap(fileName, wxBITMAP_TYPE_BMP);
    }
    else if(fileName.substr(fileName.find_last_of(".") + 1) == "jpeg")
    {
        mModel->LoadBitMap(fileName, wxBITMAP_TYPE_JPEG);
    }
    mPanel->PaintNow();
    UpdateRedoUndoButtons();
}
...
{
	// TODO
    if(mModel->GetSelectShape() != nullptr)
    {
        mModel->SetSelectShape();
    }
    mPanel->PaintNow();
    UpdateUnselect();
}
...
{
	// TODO
    wxTextEntryDialog dlg(this, "Enter an integer value from 1-10:");
    wxTextValidator numbers(wxFILTER_INCLUDE_LIST);
    wxArrayString list;
    for(int i = 1; i < 11; i++)
    {
        list.Add(std::to_string(i));
    }
    numbers.SetIncludes(list);
    dlg.SetTextValidator(numbers);
    if (dlg.ShowModal() == wxID_OK)
    {
        ...
        mModel->SetPenWidth(wxAtoi(value));
        if(mModel->GetSelectShape() != nullptr)
        {
            mModel->FinalizeCommand();
            mPanel->PaintNow();
...
void PaintFrame::OnMouseButton(wxMouseEvent& event)
{
	...
	else if (event.LeftUp())
	{
        ...
        if(mModel->MouseOnSelectShape(event.GetPosition()))
        {
...
void PaintFrame::UpdateRedoUndoButtons()
{
    if(mModel->CanRedo())
    {
